...
if __name__ == "__main__":
    ...
    H = np.array([[1.0, 0.8],
                  [0.1, 0.3]])       # System dynamics matrix
    M = np.array([[0.3, 0.1],
                  [0.8, 1.0]])       # Control input matrix
    sigma = 0.5 * np.eye(2)         # Noise matrix (scaled identity)
    C = 10 * np.eye(2)              # State cost matrix (scaled identity)
    D = 0.005 * np.eye(2)           # Control cost matrix (scaled identity)
    R = 100 * np.eye(2)             # Terminal cost matrix (scaled identity)
    T = 0.5                         # Terminal time
